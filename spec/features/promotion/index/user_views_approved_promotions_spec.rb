...
feature 'Admin views pending promotions on index' do
  ...
  scenario 'and there is 1 preview on index' do
    ...
    expect(page).to have_content("Aprovada por: #{promotion.curator.email}")
    expect(page).not_to have_link 'Aprovar'
  end

  scenario 'and there are 2 previews on index' do
    login_as(user, scope: :user)
    promotions = create_list :promotion, 2, :with_product_category, :approved, creator: creator
    ...
    expect(page).to have_content('Aprovada', count: 2)
